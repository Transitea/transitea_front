...
> ⚠️ La configuration nginx embarquée dans l'image (`default.conf`) proxifie `/api/` vers `api.transitea.fr`, le domaine du backend en **production** — elle ne fonctionne donc pas pour joindre un backend local. Pour tester le conteneur frontend contre un backend lancé en local (voir [../transitea_core/transitea/README.md](../transitea_core/transitea/README.md)), utiliser plutôt [../lancement_local.sh](../lancement_local.sh) (ou [../LANCEMENT_LOCAL.txt](../LANCEMENT_LOCAL.txt) pour le détail des commandes), qui monte à la place `nginx.local.conf` (fourni dans ce dossier) et lance les 3 conteneurs en `--network host` — nécessaire suite à un bug de connectivité conteneur-à-conteneur observé sur les réseaux Docker "bridge" définis par l'utilisateur dans certains environnements (Docker natif sous WSL2).
